//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by Entitas.CodeGeneration.Plugins.ComponentEntityApiGenerator.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
public partial class GameEntity {

    public Components.Target target { get { return (Components.Target)GetComponent(GameComponentsLookup.Target); } }
    public bool hasTarget { get { return HasComponent(GameComponentsLookup.Target); } }

    public GameEntity AddTarget(UnityEngine.Vector3 newValue) {
        var index = GameComponentsLookup.Target;
        var component = (Components.Target)CreateComponent(index, typeof(Components.Target));
        component.value = newValue;
        AddComponent(index, component);
        return this;
    }

    public GameEntity ReplaceTarget(UnityEngine.Vector3 newValue) {
        var index = GameComponentsLookup.Target;
        var component = (Components.Target)CreateComponent(index, typeof(Components.Target));
        component.value = newValue;
        ReplaceComponent(index, component);
        return this;
    }

    public GameEntity RemoveTarget() {
        RemoveComponent(GameComponentsLookup.Target);
        return this;
    }
}

//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by Entitas.CodeGeneration.Plugins.ComponentEntityApiGenerator.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
public partial class GameEntity {

    public Components.Target target { get { return (Components.Target)GetComponent(GameComponentsLookup.Target); } }
    public bool hasTarget { get { return HasComponent(GameComponentsLookup.Target); } }

    public void AddTarget(UnityEngine.Vector3 newValue) {
        var index = GameComponentsLookup.Target;
        var component = (Components.Target)CreateComponent(index, typeof(Components.Target));
        component.value = newValue;
        AddComponent(index, component);
    }

    public void ReplaceTarget(UnityEngine.Vector3 newValue) {
        var index = GameComponentsLookup.Target;
        var component = (Components.Target)CreateComponent(index, typeof(Components.Target));
        component.value = newValue;
        ReplaceComponent(index, component);
    }

    public void RemoveTarget() {
        RemoveComponent(GameComponentsLookup.Target);
    }
}

//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by Entitas.CodeGeneration.Plugins.ComponentMatcherApiGenerator.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
public sealed partial class GameMatcher {

    static Entitas.IMatcher<GameEntity> _matcherTarget;

    public static Entitas.IMatcher<GameEntity> Target {
        get {
            if (_matcherTarget == null) {
                var matcher = (Entitas.Matcher<GameEntity>)Entitas.Matcher<GameEntity>.AllOf(GameComponentsLookup.Target);
                matcher.componentNames = GameComponentsLookup.componentNames;
                _matcherTarget = matcher;
            }

            return _matcherTarget;
        }
    }
}
